...
这是一个用 Python 编写的口琴曲谱播放器。当前版本是 v0.3.0，可以读取简单的数字曲谱，计算每个音符需要使用的键盘按键、鼠标修饰键和持续时间，并支持静态预览和按 BPM 计时的终端预演。
...
程序启动后会一直等待：按 `F8` 后倒计时 3 秒并开始预演，按 `F9` 可在倒计时或预演中立即停止；完成或停止后可以再次按 `F8`。按 `Ctrl+C` 退出程序。
...
如果 F8 或 F9 已被其他软件占用，程序会报告无法注册全局热键。当前计时模式只输出“开始”和“释放”等文字，**不会按下真实键盘或鼠标，也不会向游戏发送输入**。
...
```powershell
python -X utf8 -m harmonica_player examples/twinkle_twinkle.song
```

再启动计时预演：

```powershell
python -X utf8 -m harmonica_player examples/twinkle_twinkle.song --timed
```
...
1. 试听第一首练习曲，根据实际体验调整曲谱格式。
2. 增加暂停、恢复和曲目选择功能。
3. 加入包含降调、半音和升调的练习曲。
4. 保持计时控制与输出模块可替换，为真实输入模块做准备。
